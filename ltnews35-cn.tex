...
但是，标记也可以用于其他目的。新的内核模块为互相独立的 mark 的使用提供了一种通用机制。
...
\LaTeX{} 中处理选项的经典方法是使用 \cs{ProcessOptions}，将列表中的每项视为一个字符串。
许多包作者试图通过将每项视为键值对（keyval）来扩展这种处理。
迄今为止，这需要使用额外的宏包，例如 \pkg{kvoptions}。
...
为了支持为此机制创建键值选项，添加了新命令 \cs{DeclareKeys}。
