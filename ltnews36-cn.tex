...
为了允许扩展核心 \LaTeX{} 语法，\pkg{ltcmd} 现在支持在获取参数时使用 \texttt{={...}} 修饰符。此修饰符告诉 \LaTeX{} 应将参数作为一组键/值传递给底层代码。
如果参数 \zhquote{看起来} 并不像一组键/值，它将会转换为单个键/值对，这个键/值对其键名为 \texttt{=} 所指定的那个，其值为这个参数所获取的值。例如，\cs{caption} 命令可以被定义为
\begin{verbatim}
...
万一含有 \texttt{=} 的文字被错误地解释为了键值参数，可以使用一个大括号对来\zhquote{隐藏}它（正如你在其它场合使用时一样）。
\begin{verbatim}
\caption[{Use of = signs}]
        {Use of = signs in optional arguments}
\end{verbatim}
然而，由于数学模式中的 \texttt{=} 总是被忽略了，这种情形应该很少见。
...
\section{给字体宏包开发者的注记 A note for font package developers}
...
当 \pkg{babel} 宏包加载时，\cs{MakeUppercase}、\cs{MakeLowercase} 和 \cs{MakeTitlecase} 现在可以自动检测当前的语言。你也可以通过指定一个可选的键值参数来切换语言。目前仅支持 \texttt{locale} 这一个键，键 \texttt{lang} 是它的另一个名字。其目的是允许使用 BCP-47 标签来局部地切换语言。
...
\subsection{检测嵌套的 \texttt{minipage} 环境 Detect nested \texttt{minipage} environments}

嵌套使用 \texttt{minipage} 仅部分被支持，并且可能导致不正确的输出，例如盒子出现 overfull，或者脚注出现在错误的位置，这种情况发生时不会有警告。然而，现在如果你嵌套使用在一个已经包含脚注的 \texttt{minipage} 中时，将会有警告。
...
\subsection{宏包选项中健壮的命令 Robust commands in package options}

在使用上一个发行版中添加的标准键值选项处理机制或其它宏包中提供的相似特性时，用户可能想这样设置宏包选项：\verb|[font=\bfseries]|。先前这样做会出现内部错误由于这选项列表被 \verb|\edef| 展开了。现在已经改为使用已有的 \verb|\protected@edef| 以使得 \LaTeX{} 中健壮的命令能安全的作为键值选项。
...
\subsection{改进 \pkg{l3docstrip} 与 \pkg{docstrip} 的集成 Improve \pkg{l3docstrip} integration into \pkg{docstrip}}
...
\subsection{改进 \LuaTeX\ 回调的效率 \LuaTeX\ callback efficiency improvement}
...
\subsection{防止 \TeX 丢失 \cs{smash} Prevent \TeX{} from losing a \cs{smash}}
...
\end{verbatim}
在分母中的 $2$ 并不总是在相同的垂直位置，由于第二个 \cs{smash} 在 rebox 时被忽略了。
...
\subsection{当使用键值处理类选项时报告未使用的全局选项 Reporting of unused global options when using key/value processing}
...
\section{\pkg{graphics} 类别中的修改 Changes to packages in the \pkg{graphics} category}

\subsection{修复 \cs{mathcolor} 的错误 Fix a \cs{mathcolor} bug}
...
\section{\pkg{tools} 类别中的修改 Changes to packages in the \pkg{tools} category}
